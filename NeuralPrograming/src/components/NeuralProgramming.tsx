...
const BridgingProtocols = ({ ibqos, onUpdateIBQOS, onNudge }: { ibqos?: IBQOS; onUpdateIBQOS?: (ibqos: IBQOS) => void; onNudge?: (id: number) => void }) => {
  ...
  const handleAgentQProtocolGeneration = async () => {
    ...
    // Simulate AgentQ processing
    await new Promise(resolve => setTimeout(resolve, 2000));

    const newProtocol = `// Generated by AgentQ - Neural Protocol Bridge\n// Basis: ${linkType} link between #${nodeA} and #${nodeB} with strength ${strength.toFixed(2)}\n\nfunction executeNeuralProtocol() {\n  console.log('Bridging Nodes #${nodeA} and #${nodeB}');\n  // Protocol logic for ${linkType} link\n  const bridgeHandle = establishBridge({ source: ${nodeA}, target: ${nodeB}, type: '${linkType}', strength: ${strength.toFixed(2)} });\n  bridgeHandle.activate();\n  console.log('Protocol successfully operational.');\n}`;
    ...
    >
      ...
      <div className="bg-black/40 border border-white/10 rounded-[2.5rem] p-8 flex flex-col gap-6 mt-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={handleAgentQProtocolGeneration}
              disabled={isGenerating}
              className="px-6 py-2 bg-purple-500/10 border border-purple-500/40 text-purple-400 hover:bg-purple-500 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
            >
              <Sparkles className="w-3 h-3" /> {isGenerating ? "Generating..." : "Generate with AgentQ"}
            </button>
            <button 
